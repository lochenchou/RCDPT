...
class Fusion_multi(nn.Module):
    def __init__(self, resample_dim):
        super(Fusion_multi, self).__init__()
        self.res_conv1_RGB = ResidualConvUnit(resample_dim)
        self.res_conv1_radar = ResidualConvUnit(resample_dim)
        self.res_conv2 = ResidualConvUnit(resample_dim)
        #self.resample = nn.ConvTranspose2d(resample_dim, resample_dim, kernel_size=2, stride=2, padding=0, bias=True, dilation=1, groups=1)

    def forward(self, rgb, radar, previous_stage=None):
        if previous_stage == None:
            previous_stage = torch.zeros_like(rgb)
        output_stage1_rgb = self.res_conv1_RGB(rgb)
        output_stage1_radar = self.res_conv1_radar(radar)
        
        x = output_stage1_rgb + output_stage1_radar + previous_stage
        output_stage2 = self.res_conv2(x)
        output_stage2 = nn.functional.interpolate(output_stage2, scale_factor=2, mode="bilinear", align_corners=True)
        return output_stage2