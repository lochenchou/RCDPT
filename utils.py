...
def colored_depthmap(depth, d_min=0, d_max=80):
    if d_min is None:
        d_min = np.min(depth)
    if d_max is None:
        d_max = np.max(depth)
...
def merge_RGBandRADAR(_input, depth_pred):
    ...
    img_merge = np.hstack([_input, depth_pred_col])
...
def batch_merge_into_row_radar2(input, radar, depth_target, depth_pred1, depth_pred2):
    ...
    for i in range(N):
        ...
        _radar = radar[i,:,:,:]
        _depth_target = depth_target[i,:,:]
        _depth_pred1 = depth_pred1[i,:,:]
        _depth_pred2 = depth_pred2[i,:,:]
        _input = 256 * np.transpose(_input.cpu().numpy(), (1, 2, 0))  # H, W, C
        _radar = np.squeeze(_radar.cpu().numpy())
        _depth_target = np.squeeze(_depth_target.cpu().numpy())
        _depth_pred1 = np.squeeze(_depth_pred1.data.cpu().numpy())
        _depth_pred2 = np.squeeze(_depth_pred2.data.cpu().numpy())
        d_min = 1e-3
        d_max = 80
        _radar = colored_depthmap(_radar, d_min, d_max)
        _depth_target = colored_depthmap(_depth_target, d_min, d_max)
        _depth_pred1 = colored_depthmap(_depth_pred1, d_min, d_max)
        _depth_pred2 = colored_depthmap(_depth_pred2, d_min, d_max)
        if i==0:
            img_merge = np.hstack([_input, _radar, _depth_target, _depth_pred1, _depth_pred2])
        else:
            row = np.hstack([_input, _radar, _depth_target, _depth_pred1, _depth_pred2])
            img_merge = add_row(img_merge, row)
...
def merge_into_row_with_radar2(_input, radar, depth_target, depth_pred1, depth_pred2):
    ...
    if depth_target.dim() == 4:
        depth_target = depth_target[0,:,:]
    if depth_pred1.dim() == 4:
        depth_pred1 = depth_pred1[0,:,:]
    if depth_pred2.dim() == 4:
        depth_pred2 = depth_pred2[0,:,:]
    
    rgb = 255 * np.transpose(np.squeeze(_input.cpu().numpy()), (1, 2, 0))  # H, W, C
    radar_cpu = np.squeeze(radar.cpu().numpy())
    depth_target_cpu = np.squeeze(depth_target.cpu().numpy())
    depth_pred1_cpu = np.squeeze(depth_pred1.data.cpu().numpy())
    depth_pred2_cpu = np.squeeze(depth_pred2.data.cpu().numpy())
    d_min = 1e-3
    d_max = 80
    radar_col = colored_depthmap(radar_cpu, d_min, d_max)
    depth_target_col = colored_depthmap(depth_target_cpu, d_min, d_max)
    depth_pred1_col = colored_depthmap(depth_pred1_cpu, d_min, d_max)
    depth_pred2_col = colored_depthmap(depth_pred2_cpu, d_min, d_max)

    img_merge = np.hstack([rgb, radar_col, depth_target_col, depth_pred1_col, depth_pred2_col])
